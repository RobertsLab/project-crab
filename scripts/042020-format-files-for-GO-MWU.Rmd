...
This means that I need an annnotated list of the 2019 crab dataset from BLAST
2. .csv file: two columns - gene IDs and continuous measure of significance, like p-values. 
These measures of significance need to be associated with GO enrichment. Need to be kME value, p-value, or dN/dS value... they come from DESeq? Or are can they be the p-values part of BLAST output? 
I think they need to be from `DESeq` or `edgeR` because their needs to be a component of up and down regulation, enrichment, which you can only get using `DESeq` (I think?) and definitely `edgeR`
